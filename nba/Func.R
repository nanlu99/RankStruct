...
##dynamic programming##
#######################
dpFunc=function(ADtmp,Pob,PobLess,gammals,lambdals){
print("begin dp for this CV!")
...
for(l in l_cand){
if(l>=r){break}
inter=t0save[(l*mmept+1):(r*mmept)]
#rep(((l:(r-1))/(mmcpt+1)),each=mmept)+((1:mmept)/(mmept+1))/(mmcpt+1) 
#(l*mmept+1):(r*mmept)  #l:r
PobLR=Pob[,,(l*mmept+1):(r*mmept)] #
#interLess=m*nitem*( l+(1:mmgpt)/(1+mmgpt)*(r-l) )/(mmcpt+1) 
PobLRLess=PobLess[[l+1]][[r]]

estimate=opt_group(ADtmp,PobLRLess,PobLR,inter,l,r) #group for Pob on inter/mm0
#print(estimate)
...
print(r)
if(r==floor(mmcpt/2)){
print("dp finished half, just wait, just a bit slow...")}
}
...
cp_Est<-ttsep[cp_est+1] #cp_Est<-cp_est/(mmcpt+1) #change point estimation -- [0,1]
print(cp_Est)
...
opt_group=function(ADtmp,PobLRLess,PobLR,interval,l,r){
#print(interval)
inter_len=length(interval)
adglre=ADGLCV(ADtmp,PobLRLess,interval,l,r)
hatpi=adglre[[1]]
...
########################
ADGLCV=function(ADtmp,Pobtrain,interval,l,r){ 
#1.hatpi; 2.hatgp
P=Pobtrain #[,,interval]
...
piref=KRCref(ADtmp,interval,l,r,gre)
REtemp=list()
REtemp[[1]]=piref
REtemp[[2]]=gre
#REtemp[[3]]=oddy
